...
function RoleLabel(props: { role: ChatMessage['role']; streaming?: boolean; meta?: string; spinnerFrame: number }) {
		...
		<box position="absolute" left={0} right={0} top={0} bottom={0} backgroundColor={c.bgOverlay}>
			<box position="absolute" left={ml} top={mt} width={mw} height={mh} backgroundColor={c.bgChat}>
					...
					{items.length === 0 ? (
						...
						items.slice(0, listH).map((item, i) => (
							<box key={item.id} backgroundColor={i === selected ? c.bgThink : undefined}>
								<text fg={i === selected ? c.accent : c.text}>{item.label.padEnd(12)}</text>
				...
				<box paddingX={2} paddingTop={1}>
					<text fg={c.dim}>
						{'\u2191\u2193'} navigate {'\u23CE'} {footerHint || 'select'} esc {prefix ? 'back' : 'close'}
					</text>
				</box>
			</box>
		</box>
...
function App() {
	...
	// -- refs --
	const agentRef = useRef(new Agent({ model: MODEL, systemPrompt: SYSTEM_PROMPT, tools }));
	...
	const cmdItems: CmdItem[] = useMemo(() => {
		return [
			...
			{ id: 'debug', label: 'debug', desc: 'Toggle debug panel', action: () => setShowDebug((s) => !s) },
			{
				id: 'sessions',
				label: 'sessions',
				desc: `Browse saved conversations (${currentSessions.length})`,
				navigate: true,
				action: () => {
					setCmdView('sessions');
					setCmdFilter('');
	...
	const filteredItems = useMemo(() => {
		...
		return displayItems.filter((item) => item.label.toLowerCase().includes(q) || item.desc.toLowerCase().includes(q));
		...
		<box flexDirection="column" height={termHeight}>
			...
			>
				<box flexDirection="row" gap={1}>
					<text fg={c.accent}>
						<strong>Spectra Chat</strong>
					</text>
					{copiedMsg && <text fg={c.success}>[Copied!]</text>}
					{tps !== null && (
						<text fg={c.tps}>
							{tps.toFixed(1)} {tpsIsFallback ? 'chunks/s' : 'tok/s'}
						</text>
					)}
