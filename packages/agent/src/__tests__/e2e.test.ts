...
import { z } from 'zod';
import type { Model, Message, AssistantMessage, AssistantMessageEvent } from '@mohanscodex/spectra-ai';
...
describe('Agent E2E - Advanced Features', () => {
	it('should support beforeToolCall hook', async () => {
		...
		const toolEnd = agent.messages.find((m: Message) => m.role === 'toolResult' && m.toolName === 'sensitive_tool');
		expect(toolEnd).toBeDefined();
