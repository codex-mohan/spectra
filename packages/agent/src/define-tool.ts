import type { TextContent, ImageContent } from '@mohanscodex/spectra-ai';
import type { AgentTool, ToolResult, ToolUpdateCallback } from './types.js';
import { z } from 'zod';
import { zodToJsonSchema } from 'zod-to-json-schema';

export function defineTool<T extends z.ZodType, TDetails = unknown>(schema: {
	name: string;
	label?: string;
	description: string;
	parameters: T;
	...
	return {
		...
		prepareArguments: (args: unknown): Record<string, unknown> => {
			...
			if (!result.success) {
				throw new Error(`Invalid arguments for tool "${schema.name}": ${result.error.message}`);
			}
