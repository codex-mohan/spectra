...
export class Editor implements Component {
  ...
  handleInput(data: string): void {
    ...
    // Arrow keys
    if (data === "\x1b[D") { this.cursor = Math.max(0, this.cursor - 1); this.invalidate(); return; }
    if (data === "\x1b[C") { this.cursor = Math.min(this.value.length, this.cursor + 1); this.invalidate(); return; }

    // Home/End
    if (data === "\x1b[H" || data === "\x1b[1~") { this.cursor = 0; this.invalidate(); return; }
    if (data === "\x1b[F" || data === "\x1b[4~") { this.cursor = this.value.length; this.invalidate(); return; }

    // Ctrl+A / Ctrl+E
    if (data === "\x01") { this.cursor = 0; this.invalidate(); return; }
    if (data === "\x05") { this.cursor = this.value.length; this.invalidate(); return; }
