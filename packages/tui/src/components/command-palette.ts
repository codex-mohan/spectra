...
export class CommandPalette implements Component, Focusable {
  ...
  render(width: number): string[] {
    ...
    let inputLine = colors.primary("\u276F ") + (this.query ? this.query.slice(0, this.cursor) + cursorMarker + this.query.slice(this.cursor) : colors.muted(this.placeholder));
    ...
    for (let i = start; i < end; i++) {
      ...
      if (item.shortcut) {
        ...
        const remaining = innerWidth - spacing.listPaddingX * 2 - visibleWidth(line) - visibleWidth(shortcutStr) - 2;
        if (remaining > 0) {
    ...
    if (this.filtered.length === 0) {
      const noResults = " ".repeat(spacing.listPaddingX) + colors.dim("No results found") + " ".repeat(Math.max(0, innerWidth - 18));
