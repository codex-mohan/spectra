...
function updateTrackerFromText(text: string, tracker: AnsiCodeTracker): void {
  ...
  while (i < text.length) {
    ...
    if (ansi) { tracker.process(ansi.code); i += ansi.length; }
    else i++;
  }
}
...
function splitIntoTokensWithAnsi(text: string): string[] {
  ...
  while (i < text.length) {
    ...
    if (ansiResult) { pendingAnsi += ansiResult.code; i += ansiResult.length; continue; }
    const char = text[i];
    const charIsSpace = char === " ";
    if (charIsSpace !== inWhitespace && current) { tokens.push(current); current = ""; }
...
function breakLongWord(word: string, width: number, tracker: AnsiCodeTracker): string[] {
  ...
  while (i < word.length) {
    ...
    if (ansiResult) { segments.push({ type: "ansi", value: ansiResult.code }); i += ansiResult.length; }
    else {
      let end = i;
      while (end < word.length) { const nextAnsi = extractAnsiCode(word, end); if (nextAnsi) break; end++; }
  ...
  for (const seg of segments) {
    if (seg.type === "ansi") { currentLine += seg.value; tracker.process(seg.value); continue; }
...
export function extractSegments(
  line: string, beforeEnd: number, afterStart: number, afterLen: number, strictAfter = false,
  ...
  while (i < line.length) {
    ...
    for (const { segment } of segmenter.segment(line.slice(i, textEnd))) {
      ...
      if (currentCol < beforeEnd) {
        if (pendingAnsiBefore) { before += pendingAnsiBefore; pendingAnsiBefore = ""; }
        before += segment;
        beforeWidth += w;
      } else if (currentCol >= afterStart && currentCol < afterEnd) {
        const fits = !strictAfter || currentCol + w <= afterEnd;
        if (fits) {
          if (!afterStarted) { after += pooledStyleTracker.getActiveCodes(); afterStarted = true; }
          after += segment;
          afterWidth += w;
        }
      }
