		...
		if (opt.row !== undefined) {
			row =
				typeof opt.row === 'string'
					? marginTop + Math.floor((Math.max(0, availHeight - effectiveHeight) * parseFloat(opt.row)) / 100)
					: opt.row;
		} else {
		...
		if (opt.col !== undefined) {
			col =
				typeof opt.col === 'string'
					? marginLeft + Math.floor((Math.max(0, availWidth - width) * parseFloat(opt.col)) / 100)
					: opt.col;
		} else {
	...
	private doRender(): void {
		...
		for (let i = firstChanged; i <= renderEnd; i++) {
			...
			if (!isImageLine(line) && visibleWidth(line) > width) {
				this.stop();
				throw new Error(
					`Rendered line ${i} exceeds terminal width (${visibleWidth(line)} > ${width}). Use visibleWidth() and truncateToWidth() to truncate lines.`,
				);
			}
			buffer += line;
		}
