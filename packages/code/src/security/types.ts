...
export type WriteGuardResult = { ok: true } | { ok: false; reason: string; warning?: boolean; blocked?: boolean };

export type DoomLoopResult = { ok: true } | { ok: false; action: 'warn' | 'stop'; message: string };

export type PathSafetyResult = { ok: true; resolvedPath: string; displayPath: string } | { ok: false; reason: string };
