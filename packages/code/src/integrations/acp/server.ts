...
export class ACPAdapter {
  ...
  private async handle(req: JsonRpcRequest): Promise<void> {
    switch (req.method) {
      ...
      case "session/new": return this.onSessionNew(req);
      case "session/prompt": return this.onSessionPrompt(req);
      case "session/cancel": return this.onSessionCancel(req);
  ...
  private async onSessionNew(req: JsonRpcRequest): Promise<void> {
    ...
    try {
      ...
      if (!def) { error(req.id!, -32603, `Unknown agent: ${agentName}`); return; }
  ...
  private async onSessionPrompt(req: JsonRpcRequest): Promise<void> {
    ...
    if (!session) { error(req.id!, -32602, `Session not found: ${params.sessionId}`); return; }

    const text = typeof params.message?.content === "string"
    ...
    switch (event.type) {
      ...
      case "tool_execution_end":
        notify("session/update", {
          sessionId,
          status: "running",
          toolCall: { id: event.toolCallId, name: event.toolName, result: event.result, status: event.isError ? "error" : "completed" },
        });
        break;
