...
export function PromptBar(props: PromptBarProps) {
	...
	useEffect(() => {
		if (initialValue && initialValue.length >= COMPACT_THRESHOLD) {
			setInternalText(initialValue);
			setIsCompact(true);
		}
	}, [initialValue]);
	...
	const handleTextChange = useCallback(
		(text: string) => {
			...
			if (!isCompact && text.length >= COMPACT_THRESHOLD) {
				setIsCompact(true);
			}
			onTextChange?.(text);
		},
		[isCompact, onTextChange],
	);
			...
			<box flexDirection="row">
				...
				>
					<box flexDirection="column" flexGrow={1} paddingLeft={2}>
						{isCompact ? (
							<box flexDirection="row" alignItems="center" height={1} gap={2}>
								<text fg={c.accent}>›</text>
								<text fg={c.warn}>
									{internalText.length.toLocaleString()} chars pasted — enter to send
								</text>
								<text fg={c.dim}>·</text>
								<text fg={c.accent}>esc to edit</text>
							</box>
						) : (
								...
								<box flexDirection="row" flexGrow={1} gap={1}>
									...
									<box flexGrow={1}>
										<textarea
											...
											maxHeight={6}
											width={'100%'}
											initialValue={internalText || initialValue}
											keyBindings={[
											...
											onContentChange={() => {
												if (textareaRef.current) {
													handleTextChange(textareaRef.current.plainText);
												}
											}}
