  ...
  useEffect(() => {
    registerHandler((key: any) => {
      ...
      if (key.name === "return" || key.name === "enter") {
        if (filtered[sel]) onAgentSelected(filtered[sel].name); return
      }
      ...
      <box position="absolute" left={ml} top={mt} width={mw} height={mh} backgroundColor={c.bgCard}>
        ...
        <scrollbox ref={(r: any) => { scrollRef.current = r }}
          ...
          <box flexDirection="column">
            {filtered.length === 0 ? (
              <box height={1} paddingX={1}><text fg={c.dim}>No matching agents</text></box>
            ) : filtered.map((item, i) => {
              const isSelected = i === sel
              const isCurrent = item.name === currentAgent
              return (
                <box key={item.name} id={`agent-${item.name}`} height={2} paddingLeft={2} paddingRight={1}
                  backgroundColor={isSelected ? c.bgSelect : c.bgCard}>
                  <box flexDirection="row" alignItems="center" gap={1}>
                    <text fg={isCurrent ? c.accent : c.dim}>{isCurrent ? "●" : " "}</text>
                    <text fg={isSelected ? c.accent : isCurrent ? c.text : c.dim}>{titlecase(item.name)}</text>
                  </box>
                  <text fg={c.dim} overflow="hidden" wrapMode="none">{item.def?.description}</text>
                </box>
              )
            })}
          </box>
