...
export function ThinkingEffortDialog(props: ThinkingEffortDialogProps) {
	...
	const filtered = useMemo(() => {
		...
		return efforts.map((e, idx) => ({ ...e, idx })).filter((e) => e.label.includes(q) || e.effort.includes(q));
	}, [efforts, filter]);
	...
	useEffect(() => {
		registerHandler((key: any) => {
			...
			if (key.name === 'return' || key.name === 'enter') {
				if (filtered[sel]) onEffortSelected(filtered[sel].effort);
				return;
			}
			if (key.name === 'escape') {
				onClose();
					...
					<box flexDirection="column">
						{filtered.length === 0 ? (
							<box height={1} paddingX={1}>
								<text fg={c.dim}>No matching efforts</text>
							</box>
						) : (
							filtered.map((item, i) => {
