...
export function SessionList(props: SessionListProps) {
	...
	const sessions = useMemo(() => {
		...
		if (!q) return list;
		return list.filter(
			(s) => s.title.toLowerCase().includes(q) || s.model.toLowerCase().includes(q) || s.id.includes(q),
		);
	...
	useEffect(() => {
		registerHandler((key: any) => {
			...
			if (key.name === 'up') {
				setSel((p) => (p > 0 ? p - 1 : sessions.length - 1));
				return;
			}
			if (key.name === 'down') {
				setSel((p) => (p < sessions.length - 1 ? p + 1 : 0));
				return;
			}
			if (key.name === 'backspace') {
	...
	const hint =
		mode === 'delete'
			? confirmDelete
				? `Delete "${sessions[sel]?.title.slice(0, 30)}"? enter confirm · esc cancel`
				: '↑↓ navigate · enter select to delete · esc close'
			: mode === 'rename'
				? renameInput
					? `enter confirm · esc cancel`
					: '↑↓ navigate · enter select to rename · esc close'
					...
					<box flexDirection="column">
						{rows.length === 0 ? (
							<box height={1} paddingX={1} backgroundColor={c.bgCard}>
								<text fg={c.dim}>No sessions</text>
							</box>
						) : (
							rows
						)}
