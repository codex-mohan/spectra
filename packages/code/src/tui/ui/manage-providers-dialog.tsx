...
export function ManageProvidersDialog(props: {
  ...
  useEffect(() => {
    registerHandler((key: any) => {
      if (key.name === "escape") {
        if (mode === "add" || mode === "edit") { setMode("list"); setErr(""); setForm({ id: "", name: "", baseUrl: "", apiKey: "", models: ["", "", ""] }); return }
        if (mode === "delete-confirm") { setConfirmDeleteId(null); setMode("list"); return }
        onClose()
        return
      }
      if (mode === "list") {
        if (key.name === "up") { setListSel(p => Math.max(0, p - 1)); return }
        if (key.name === "down") { setListSel(p => Math.min(providerEntries.length - 1, p + 1)); return }
        if (key.name === "a" || key.name === "A") { startAdd(); return }
        if ((key.name === "e" || key.name === "E") && providerEntries.length > 0) { startEdit(providerEntries[listSel]?.[0]); return }
        if ((key.name === "d" || key.name === "D") && providerEntries.length > 0) { setConfirmDeleteId(providerEntries[listSel]?.[0]); setMode("delete-confirm"); return }
        return
      }
      if (mode === "delete-confirm") {
        if (key.name === "return" || key.name === "enter") { handleDeleteConfirm(); return }
        return
      }
  ...
  const handleSave = () => {
    setErr("")
    if (!form.id.trim()) { setErr("Provider ID is required"); return }
    ...
    setFormField("id")
    setTimeout(() => { setMode("list"); setSuccess("") }, 800)
  }
  ...
  const startEdit = (id: string) => {
    ...
    setForm({
      ...
      apiKey: cfg.apiKey || "",
      models: cfg.models
        ? Object.keys(cfg.models).concat(["", "", ""]).slice(0, 3)
        : ["", "", ""],
    })
    setFormField("name")
    setMode("edit")
  ...
  if (mode === "list") {
    rows.push(
      <box paddingX={2} paddingTop={1} height={1} flexDirection="row" justifyContent="space-between">
        <box flexDirection="row" gap={1}><text fg={c.accent}>{">"}</text><text fg={c.text}>Manage Providers</text></box>
        <box flexDirection="row" height={1}><text fg={c.dim}>esc close</text></box>
      </box>,
      <box height={1} paddingX={2}><text fg={c.border}>{"─".repeat(mw - 4)}</text></box>,
    )
    if (providerEntries.length === 0) {
      rows.push(<box height={2} paddingX={2} flexDirection="column" gap={1}>
      ...
      for (let i = 0; i < providerEntries.length; i++) {
        ...
        rows.push(
          <box key={id} height={2} paddingX={2} flexDirection="column" gap={0}
            backgroundColor={isSel ? c.bgSelect : undefined}>
            <box flexDirection="row" justifyContent="space-between">
              <text fg={isSel ? c.accent : c.text} attributes={isSel ? 1 : 0}>{cfg.name || id}</text>
    ...
    rows.push(
      <box height={1} paddingX={2}><text fg={c.border}>{"─".repeat(mw - 4)}</text></box>,
      <box height={1} paddingX={2}><text fg={c.accent}>a add provider</text></box>,
      <box height={1} paddingX={2}><text fg={c.dim}>↑↓ navigate · a add · e edit · d delete · esc close</text></box>,
    )
    if (success) rows.push(<box height={1} paddingX={2}><text fg={c.success}>{success}</text></box>)
  } else if (mode === "delete-confirm") {
    rows.push(
      <box paddingX={2} paddingTop={1} height={1}><text fg={c.error} attributes={1}>Delete Provider</text></box>,
      <box height={1} paddingX={2}><text fg={c.border}>{"─".repeat(mw - 4)}</text></box>,
      <box height={1} paddingX={2}><text fg={c.text}>Delete "{confirmDeleteId}"? This cannot be undone.</text></box>,
    ...
    rows.push(
      <box paddingX={2} paddingTop={1} height={1} flexDirection="row" justifyContent="space-between">
        <text fg={c.accent} attributes={1}>{mode === "add" ? "Add Provider" : "Edit Provider"}</text>
    ...
    for (const f of fields) {
      ...
      const value = f.key === "id" ? form.id : f.key === "name" ? form.name : f.key === "baseUrl" ? form.baseUrl : f.key === "apiKey" ? form.apiKey : f.key === "model1" ? form.models[0] : f.key === "model2" ? form.models[1] : form.models[2]
      rows.push(
        <box key={f.key} height={1} paddingX={2} flexDirection="row" gap={1}>
          <text fg={isActive ? c.accent : c.dim} width={14}>{f.label}:</text>
          <box flexGrow={1}>
    ...
    if (err) rows.push(<box height={1} paddingX={2}><text fg={c.error}>{err}</text></box>)
    if (success) rows.push(<box height={1} paddingX={2}><text fg={c.success}>{success}</text></box>)
    rows.push(
      <box height={1} paddingX={2}><text fg={c.dim}>enter save · esc cancel · tab next field</text></box>,
    )
  }

  return (
    <box position="absolute" left={0} right={0} top={0} bottom={0} backgroundColor={c.bgOverlay}>
      <box position="absolute" left={Math.floor((termWidth - mw) / 2)} top={mt} width={mw} height={mh} backgroundColor={c.bgCard} flexDirection="column">
        {rows}
      </box>
    </box>
  )
