...
export function MessageControls(props: MessageControlsProps) {
  const { message, sessionId, messages, revertPoint, onRevert, onRedo, onFork, onClose, registerHandler, termWidth, termHeight } = props
  ...
  useEffect(() => {
    registerHandler((key: any) => {
      ...
      if (key.name === "up") { setSel((p) => (p > 0 ? p - 1 : ACTIONS.length - 1)); return }
      if (key.name === "down") { setSel((p) => (p < ACTIONS.length - 1 ? p + 1 : 0)); return }
    })
  }, [sel, confirmAction, msgText, sessionId, message.id, onRevert, onRedo, onFork, onClose, registerHandler, ACTIONS])
    ...
    <box position="absolute" left={0} right={0} top={0} bottom={0} backgroundColor={c.bgOverlay}>
      <box position="absolute" left={ml} top={mt} width={mw} height={mh} backgroundColor={c.bgCard}>
        <box height={1} paddingX={2} paddingTop={1}
          ...
          <text fg={c.text} attributes={1}>Message Actions</text>
        ...
        <scrollbox ref={scrollRef} paddingX={1} maxHeight={listH} scrollY={true}
          scrollbarOptions={{ visible: false }}>
          <box flexDirection="column">
            {ACTIONS.map((action, i) => {
              const isSelected = i === sel
              return (
                <box key={action.id} id={action.id} height={1} paddingLeft={2} paddingRight={1}
                  ...
                  <text fg={c.dim} flexShrink={0}>{action.desc}</text>
                </box>
              )
            })}
          </box>
        ...
        {confirmAction && (
          <box height={1} paddingX={2} backgroundColor={c.bgBar}>
            <text fg={c.warn}>Confirm {confirmAction === "revert" ? "Revert" : confirmAction === "fork" ? "Fork" : "Action"}? Press Enter</text>
          </box>
        )}
