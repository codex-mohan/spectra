...
interface UseChatSubmitDeps {
  ...
  getOrCreateAgent: (model: string | null, provider: string | null, agent: string, customProviders: any, effort: string | undefined) => Promise<any>
...
export function useChatSubmit(deps: UseChatSubmitDeps) {
  const {
    ...
    snapshotManager, lastAgentRef, isStreamingRef, currentTurnStartRef,
  ...
  const handleSubmit = useCallback(async (text: string) => {
    ...
    try {
      const agent = await getOrCreateAgent(selectedModel, provider, selectedAgent, customProviders, thinkingEffort)
    ...
    selectedModel, provider, selectedAgent, customProviders, thinkingEffort,
    cmdItems, slashNames, getOrCreateAgent, revertPoint,
    addMessage, updateMessage, sessionStore, sessionId, agentRef,
    loadedSessionMessages, snapshotManager, lastAgentRef, isStreamingRef,
    currentTurnStartRef, currentTurnMsgIdRef,
    setIsLoading, setStatus, setRoute, setElapsedMs, setTokPerSec,
    setTokenUsage, setDraftText, setSlashSelected, setSubmitKey,
    setPromptHistory, setHistoryIdx, setInterruptKey, setRevertPoint,
    discardRevert, securityRef,
  ])
