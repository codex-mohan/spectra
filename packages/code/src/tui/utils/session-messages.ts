...
export function sdkMessagesToChatMessages(
  data: { messages: any[]; model?: string; agent?: string }
): ConvertedMessages {
...
export function sdkMessagesToLoadedMessages(data: any): { messages: Message[]; tokenUsage: { input: number; output: number } } {
  let maxInputTokens = 0
  let outputTokens = 0
