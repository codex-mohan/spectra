...
export class SessionEngine {
	...
	async run(
		...
		if (this.activeSessions.size >= (this.config.maxConcurrentSessions ?? 100)) {
			throw new Error(`Max concurrent sessions reached (${this.config.maxConcurrentSessions}).`);
		}

		if (this.config.rateLimiter) {
			const limitKey = options?.tenantId ? `${options.tenantId}:${userId}` : userId;
			const limit = await this.config.rateLimiter.checkLimit(limitKey);
			if (!limit.allowed) {
				throw new RateLimitExceededError(`Rate limit exceeded. Reset at ${limit.resetAt.toISOString()}`);
			}
		}
