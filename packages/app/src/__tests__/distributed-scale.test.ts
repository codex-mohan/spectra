...
describe("DefaultCircuitBreaker", () => {
  ...
  it("should open after threshold failures", async () => {
    ...
    for (let i = 0; i < 3; i++) {
      try { await cb.call(failing); } catch {}
    }
  ...
  it("should reopen on HALF_OPEN failure", async () => {
    ...
    try { await cb.call(vi.fn().mockRejectedValue(new Error("fail"))); } catch {}
    await new Promise((r) => setTimeout(r, 20));

    try { await cb.call(vi.fn().mockRejectedValue(new Error("fail again"))); } catch {}
    expect(cb.state).toBe("OPEN");
  });
